...
function parseInstruction({direction, distance}, {length, depth}) {
  switch (direction) {
    case 'forward': {
      return {length: length + distance, depth};
    }
    case 'down': {
      return {length, depth: depth + distance};
    }
    case 'up': {
      return {length, depth: depth - distance};
    }
...
function executeInstructions(instructions) {
  let coordinates = {length: 0, depth: 0};
